...
from dynamical_models import Lorenz_63, Lorenz_96, oregonator, Adv_Dif_1D, Adv_Dif_1D_Real
def generate_data(GD):
    ...
    # reinitialize random generator number
    ...
    elif (GD.model == 'Adv_Dif_1D_Real'):
        class catalog:
            num_integration = [];# ADAMS
            true_solution = [];
            euler_integration = []; 
            time = [];        
        x0 = np.array([GD.parameters.x0]);
        ...
        euler_sol = [x0]
        for i in range(1,len(t)):
            euler_sol.append((1+GD.dt_integration*GD.parameters.w)*euler_sol[-1])
            
        S = odeint(Adv_Dif_1D_Real,x0,np.arange(t0,t1+0.000001,GD.dt_integration),args=(GD.parameters.w,));
        catalog.time = np.arange(t0,t1+0.000001,GD.dt_integration)
        catalog.num_integration = S
        catalog.true_solution = np.array(true_sol)  
        catalog.euler_integration = np.array(euler_sol) 
